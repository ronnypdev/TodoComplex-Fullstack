...
export async function deleteItem(itemId: number): Promise<TodoItemResult[]> {
  try {
    // Delete the item from the database
    await db.delete(listItemTable).where(eq(listItemTable.id, itemId));
    // Fetch the updated list of items
    const updatedItems = await db.select().from(listItemTable);
